...
import langLogos from '../utils/langLogos'

type repoProps = {
  repo: repoType
  filterByLanguage: (topic: string) => void
}

function Repo({ repo, filterByLanguage }: repoProps) {
  const thumbnail = 'https://source.unsplash.com/random/400x300'
  const {
    name,
    clone_url,
    description,
  ...
  return (
    ...
    >
      <CardMedia
        ...
        src={langLogos[language || 'JavaScript'] || thumbnail}
        alt='main language'
      />
      <Divider />
      <CardContent
